...
- **TensorRT 加速**：YOLOv8L-Pose 模型在 RTX 5090 上推理仅需 **5ms**
- **双机架构**：运算与游戏分离，游戏机零性能损耗
- **硬件级鼠标模拟**：ESP32S3 USB HID 模拟，完全无痕
...
| 图像推理 | ~5ms | TensorRT + YOLOv8L-Pose @ RTX 5090 |
| 串口+USB | ~2ms | ESP32S3 鼠标移动执行 |
| **总延迟** | **~12ms** | 端到端延迟 |
...
### 游戏机
- **ESP32S3**：运行 Arduino 代码，模拟 USB HID 鼠标
- **连接**：HDMI 输出到采集卡，USB 连接 ESP32S3
...
1. 连接硬件：HDMI 线连接游戏机显卡到采集卡，ESP32S3 连接游戏机 USB
2. 启动程序：运行编译后的 gprs.exe
3. 选择模式：根据武器类型选择射击模式（狙击/点射/自动）
